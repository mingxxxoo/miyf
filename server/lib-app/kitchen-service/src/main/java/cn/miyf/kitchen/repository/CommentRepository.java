package cn.miyf.kitchen.repository;

import cn.miyf.kitchen.bean.entity.CommentEntity;
import com.baomidou.mybatisplus.core.mapper.BaseMapper;
import org.apache.ibatis.annotations.Mapper;
import org.apache.ibatis.annotations.Param;

import java.util.List;
import java.util.Map;

/**
 * 评价数据访问接口（MyBatis Mapper）。
 * 单表 CRUD 继承 {@link BaseMapper}；分页与评分聚合见 CommentRepository.xml。
 * 用户昵称/头像与图片 enrich、评分重算由 CommentApplicationService 编排。
 *
 * @author XieMingJie
 * @since 2026-09-04 16:41
 */
@Mapper
public interface CommentRepository extends BaseMapper<CommentEntity> {

    /**
     * 判断同一预约+菜品+用户是否已评价，用于防重复提交。
     *
     * @param orderId 预约 ID
     * @param dishId  菜品 ID
     * @param userId  用户 ID
     * @return 已存在条数
     * @history 1.00 2026-09-04 XieMingJie Created.
     */
    long countExists(@Param("orderId") Long orderId,
                     @Param("dishId") Long dishId,
                     @Param("userId") Long userId);

    /**
     * 更新评论状态（NORMAL / HIDDEN）。
     *
     * @param id     评论 ID
     * @param status 目标状态
     * @return 影响行数
     * @history 1.00 2026-09-04 XieMingJie Created.
     */
    int updateStatus(@Param("id") Long id, @Param("status") String status);

    /**
     * 菜品维度评价分页（通常仅 NORMAL），支持星级与排序。
     *
     * @param dishId 菜品 ID
     * @param rating 星级，可空
     * @param sort   排序：latest / rating_asc / rating_desc
     * @param offset 偏移
     * @param limit  条数
     * @return 评价列表
     * @history 1.00 2026-09-04 XieMingJie Created.
     */
    List<CommentEntity> selectByDishPage(@Param("dishId") Long dishId,
                                         @Param("rating") Integer rating,
                                         @Param("sort") String sort,
                                         @Param("offset") long offset,
                                         @Param("limit") long limit);

    /**
     * 菜品评价分页总数。
     *
     * @param dishId 菜品 ID
     * @param rating 星级，可空
     * @return 总数
     * @history 1.00 2026-09-04 XieMingJie Created.
     */
    long countByDishPage(@Param("dishId") Long dishId, @Param("rating") Integer rating);

    /**
     * 管理端评价分页，可按菜品、状态、星级筛选。
     *
     * @param dishId 菜品 ID，可空
     * @param status 状态，可空
     * @param rating 星级，可空
     * @param offset 偏移
     * @param limit  条数
     * @return 评价列表
     * @history 1.00 2026-09-04 XieMingJie Created.
     */
    List<CommentEntity> selectAdminPage(@Param("dishId") Long dishId,
                                        @Param("status") String status,
                                        @Param("rating") Integer rating,
                                        @Param("offset") long offset,
                                        @Param("limit") long limit);

    /**
     * 管理端评价分页总数。
     *
     * @param dishId 菜品 ID，可空
     * @param status 状态，可空
     * @param rating 星级，可空
     * @return 总数
     * @history 1.00 2026-09-04 XieMingJie Created.
     */
    long countAdminPage(@Param("dishId") Long dishId,
                        @Param("status") String status,
                        @Param("rating") Integer rating);

    /**
     * 用户本人评价分页。
     *
     * @param userId 用户 ID
     * @param offset 偏移
     * @param limit  条数
     * @return 评价列表
     * @history 1.00 2026-09-04 XieMingJie Created.
     */
    List<CommentEntity> selectByUserPage(@Param("userId") Long userId,
                                         @Param("offset") long offset,
                                         @Param("limit") long limit);

    /**
     * 用户本人评价分页总数。
     *
     * @param userId 用户 ID
     * @return 总数
     * @history 1.00 2026-09-04 XieMingJie Created.
     */
    long countByUserPage(@Param("userId") Long userId);

    /**
     * 聚合有效评价的平均分与数量，键为 avg_rating、cnt。
     *
     * @param dishId 菜品 ID
     * @return 聚合行，可能为空 Map
     * @history 1.00 2026-09-04 XieMingJie Created.
     */
    Map<String, Object> avgRating(@Param("dishId") Long dishId);

    /**
     * 有效（NORMAL）评价总数。
     *
     * @return 数量
     * @history 1.00 2026-09-04 XieMingJie Created.
     */
    long countNormal();

    /**
     * 隐藏（HIDDEN）评价总数，仪表盘待审数量可复用。
     *
     * @return 数量
     * @history 1.00 2026-09-05 XieMingJie Created.
     */
    long countHidden();

    /**
     * 有效评价按星级分组，每行含 rating、cnt。
     *
     * @return 分组行列表
     * @history 1.00 2026-09-05 XieMingJie Created.
     */
    List<Map<String, Object>> countGroupByRating();
}
