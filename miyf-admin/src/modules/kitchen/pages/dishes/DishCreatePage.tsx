...
import { categoryApi, dishApi, kitchenApi } from '@/api';
...
export default function DishCreatePage() {
  ...
  useEffect(() => {
    categoryApi
      ...
      .catch(() => setCategories([]));
    kitchenApi
      .page({ page: 1, pageSize: 200, status: 'OPEN' })
      .then((res) => setKitchens(res.records ?? []))
      .catch(() => setKitchens([]));
  }, []);
  ...
  const onFinish = async (values: Record<string, unknown>) => {
    ...
    try {
      await dishApi.create({
        kitchenId: String(values.kitchenId),
      ...
      message.success('菜品已创建（草稿）。请提交审核，通过后再上架。');
  ...
  return (
    <div className="ck-page">
      <PageHeader title="新增菜品" description="须归属厨师厨房；关键信息变更需审核，无价格字段。" />
      <Card bordered={false}>
        <Form
          form={form}
          layout="vertical"
          ...
          onFinish={onFinish}
        >
          <Form.Item name="kitchenId" label="所属厨房" rules={[{ required: true, message: '请选择厨房' }]}>
            <Select
              placeholder="选择厨师厨房"
              options={kitchens.map((k) => ({ value: k.id, label: k.name }))}
              notFoundContent="暂无营业中的厨房"
              showSearch
